...
//        11-Feb-2022 12:42:57.100 INFO [main] org.apache.catalina.startup.Catalina.load Initialization processed in 1610 ms
        String message = OffsetDateTime.now() + " " + record.getLevel() + " [" + record.getThreadID() + "] "
                + record.getSourceClassName() + " - " + record.getMessage() + NL;
...
//        return sb.toString();
    ...
    public static String getClassMethod(LogRecord record) {
        ...
        if (className == null
                || className.contains("default")    //	anonymous logger
                || className.contains("global"))    //	global logger
